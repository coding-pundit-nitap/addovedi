...
function VortexRing({ count = 2400, radius = 6, height = 1.2, primary = "#66FFFF" }: { count?: number; radius?: number; height?: number; primary?: string }) {
  const pts = useMemo(() => {
  ...
  return (
    <points ref={ref} position={[0, -0.2, 0]}>
      ...
      <pointsMaterial color={primary} size={0.05} transparent opacity={0.6} depthWrite={false} />
    </points>
  );
}

function ForwardSparks({ count = 1500, spreadX = 14, spreadY = 8, primary = "#B366FF", speedRef }: { count?: number; spreadX?: number; spreadY?: number; primary?: string; speedRef: React.MutableRefObject<number> }) {
  const ref = useRef<THREE.Points>(null!);
  ...
  return (
    <points ref={ref}>
      ...
      <pointsMaterial color={primary} size={0.06} opacity={0.35} transparent depthWrite={false} />
    </points>
  );
}

function Pulses({ primary = "#66FFFF", secondary = "#B366FF" }: { primary?: string; secondary?: string }) {
  ...
  useFrame((state) => {
    refs.current.forEach((m, i) => {
      ...
      const alpha = 1 - t / 2.4;
      m.scale.set(s, s, s);
      (m.material as THREE.MeshStandardMaterial).color.set(i % 2 === 0 ? primary : secondary);
      (m.material as THREE.MeshStandardMaterial).emissive.set(i % 2 === 0 ? primary : secondary);
  ...
  return (
    <group position={[0, -0.6, -4]}>
      {rings.map((_, i) => (
        <mesh key={i} ref={(el) => el && (refs.current[i] = el)} rotation={[Math.PI / 2, 0, 0]}>
          <ringGeometry args={[2.4, 2.5, 64]} />
          <meshStandardMaterial emissive={primary} emissiveIntensity={1.2} color={primary} opacity={0.3} transparent />
        </mesh>
      ))}
    </group>
  );
...
export default function RCParticles({ primary = "#66FFFF", secondary = "#B366FF", density = 1 }: RCParticlesProps) {
  ...
  return (
    <div className="fixed top-0 left-0 h-screen w-full pointer-events-none">
      <Canvas camera={{ position: [0, 0.2, 10], fov: 60 }}>
        ...
        <ForwardSparks primary={secondary} speedRef={speedRef} count={Math.floor(1500 * density)} />
