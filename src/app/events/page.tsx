        ...
        >
          ...
          <div className="mt-20 text-center">
            ...
            <p className="mx-auto mt-2 max-w-xl text-sm text-[#708399]">
              Gear up for the ultimate RC showdown — register now and own the track.
            </p>
        ...
        >
          {selectedItem && (
            <div className="mb-4 rounded-lg border border-[#0B1020] bg-[#030610] p-3 text-sm text-[#94a3b8]">
              <div className="flex flex-wrap items-center gap-3">
                <span className="inline-flex items-center gap-2">
                  <CalendarDays className="size-4 text-[#B366FF]" /> {activeDay.dayLabel}
                </span>
                <span className="inline-flex items-center gap-2">
                  <Clock className="size-4 text-[#6699FF]" /> {selectedItem.time}
                </span>
                <span className="inline-flex items-center gap-2">
                  <MapPin className="size-4 text-[#6699FF]" /> {selectedItem.location ?? "Main Arena"}
                </span>
              </div>
          ...
          {selectedDetails ? (
            <div className="space-y-6">
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                {selectedDetails.teamSize && (
                  <div className="rounded-lg border border-[#0B1020] bg-[#030610] p-3">
                    <div className="text-xs uppercase tracking-widest text-[#66FFFF]">Team Size</div>
                    <div className="mt-1 text-sm text-[#e2e8f0]">{selectedDetails.teamSize}</div>
                  </div>
                )}
                {selectedDetails.prizeMoney && (
                  <div className="rounded-lg border border-[#0B1020] bg-[#030610] p-3">
                    <div className="text-xs uppercase tracking-widest text-[#66FFFF]">Prize Money</div>
                    <div className="mt-1 text-sm text-[#e2e8f0]">{selectedDetails.prizeMoney}</div>
                  </div>
                )}
                {selectedDetails.registrationFee && (
                  <div className="rounded-lg border border-[#0B1020] bg-[#030610] p-3">
                    <div className="text-xs uppercase tracking-widest text-[#66FFFF]">Registration Fee</div>
                    <div className="mt-1 text-sm text-[#e2e8f0]">{selectedDetails.registrationFee}</div>
                  </div>
                )}
                {selectedDetails.resultAnnouncement && (
                  <div className="rounded-lg border border-[#0B1020] bg-[#030610] p-3">
                    <div className="text-xs uppercase tracking-widest text-[#66FFFF]">Result Announcement</div>
                    <div className="mt-1 text-sm text-[#e2e8f0]">{selectedDetails.resultAnnouncement}</div>
                  </div>
                )}
              </div>
              ...
              {selectedDetails.guidelines && selectedDetails.guidelines.length > 0 && (
                <div>
              ...
              {selectedDetails.contact && (
                <div className="rounded-lg border border-[#0B1020] bg-[#030610] p-3">
                  <div className="text-xs uppercase tracking-widest text-[#66FFFF]">Contact</div>
                  <div className="mt-1 text-sm text-[#e2e8f0]">{selectedDetails.contact}</div>
                </div>
              )}
              ...
              <div className="mt-2 flex items-center justify-end gap-2">
                ...
                <Button asChild size="sm" className="racing-button bg-[#B366FF] hover:bg-[#B366FF]/90 text-white">
                  <Link href="/register">Register Now</Link>
                </Button>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <p className="text-[#cbd5e1]">Detailed information will be available soon.</p>
              <div className="mt-2 flex items-center justify-end gap-2">
                <Button
                  variant="outline"
                  size="sm"
